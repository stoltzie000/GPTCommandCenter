...
function renderSummary(workflow) {
  ...
  addDefinition(dl, 'Status', workflow.failureCode === 'CODEX_RUNTIME_UNAVAILABLE' ? 'Codex handoff required' : (labels[workflow.status] || workflow.status));
...
async function renderHandoff(workflow) {
  const section = $('handoff'); section.replaceChildren(); const codex = workflow.codexHandoff; if (workflow.status === 'MANUAL_HANDOFF_REQUIRED' && codex) { setHidden(section, false); section.append(node('h3', 'Codex task ready')); section.append(node('p', codex.reason)); if (codex.task) { const packageText = JSON.stringify({ workflowId: workflow.id, planId: codex.planId, codexTaskArtifactId: codex.artifactId, task: codex.task, instructions: codex.instructions }, null, 2); const pre = node('pre', packageText, 'handoff-package'); const copy = node('button', 'Copy Codex task', 'secondary'); copy.onclick = async () => { try { await navigator.clipboard.writeText(packageText); copy.textContent = 'Copied'; } catch { showError('workflow-error', 'Clipboard access was unavailable; copy the displayed task manually.'); } }; section.append(pre, copy); } if (codex.submitted) { section.append(node('p', 'External Codex evidence was submitted. It remains unverified and requires manual review.', 'warning')); return; } if (codex.returnEndpoint) { const form = node('form', undefined, 'inline-form'); const output = node('textarea'); output.required = true; output.maxLength = 100000; output.rows = 8; output.placeholder = 'Paste a bounded Codex execution report as JSON'; const submit = node('button', 'Submit Codex evidence'); form.append(output, submit); form.onsubmit = async (event) => { event.preventDefault(); submit.disabled = true; try { const parsed = JSON.parse(output.value); await api(codex.returnEndpoint, { method: 'POST', body: JSON.stringify({ output: parsed }) }); await refresh(); } catch (error) { showError('workflow-error', error.message === 'Unexpected end of JSON input' ? 'Codex evidence must be valid JSON.' : error.message); submit.disabled = false; } }; section.append(node('p', 'Returned evidence is externally supplied and is not represented as verified Codex execution.', 'warning'), form); } return; }
  setHidden(section, workflow.status !== 'MANUAL_HANDOFF_REQUIRED' || !!workflow.orchestrationPlan); if (workflow.status !== 'MANUAL_HANDOFF_REQUIRED' || workflow.orchestrationPlan) return;
...
async function renderResult(workflow) {
  ...
  try { const result = await api(`/v1/workflows/${encodeURIComponent(workflow.id)}/result`); section.append(node('h3', workflow.status === 'COMPLETE' ? 'Result' : 'Outcome'));
    for (const artifact of result.artifacts || []) { const card = node('article', undefined, 'artifact'); card.append(node('strong', artifact.type || artifact.artifactType || 'Artifact'), node('span', artifact.source === 'user_submitted' || ['manual_specialist_output', 'manual_codex_output'].includes(artifact.artifactType) ? 'Externally supplied' : 'Platform evidence', 'muted')); const content = artifact.content ?? artifact.contentJson; const pre = node('pre', typeof content === 'string' ? content : JSON.stringify(content, null, 2)); card.append(pre); section.append(card); }
...
$('task-form').onsubmit = createWorkflow; $('new-workflow').onclick = () => { if (state.timer) clearInterval(state.timer); state.workflowId = null; state.workflow = null; $('workflow-panel').hidden = true; $('intake').hidden = false; $('task-form').reset(); $('task-form button[type="submit"]').disabled = false; $('intake-error').hidden = true; };
