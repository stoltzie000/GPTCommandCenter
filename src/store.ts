...
export interface PersistenceStore {
  ...
  acceptManualHandoff(workflowId:string,content:unknown,actorId:string):Workflow|Promise<Workflow>;
  acceptManualStageHandoff(workflowId:string,planId:string,planVersion:number,stageId:string,content:unknown,actorId:string):Workflow|Promise<Workflow>;
...
export class MemoryStore implements PersistenceStore,RoutingHistoryStore,RoutingResolutionStore,ClarificationStore,OverrideStore,SpecialistCatalogStore,OrchestrationPlanStore {
  ...
  async createDynamicSpecialist(specialist: Specialist){if(specialist.origin!=='DYNAMIC')throw new Error('INVALID_DYNAMIC_SPECIALIST_ORIGIN');if(this.dynamicSpecialists.has(specialist.id))return this.dynamicSpecialists.get(specialist.id)!;const catalog=this.getEffectiveSpecialistsSync();if(catalog[specialist.id])throw new Error('DYNAMIC_SPECIALIST_COLLIDES_WITH_BUILTIN');this.dynamicSpecialists.set(specialist.id,specialist);this.getEffectiveSpecialistsSync();return specialist;}
  ...
  async acceptManualHandoff(workflowId:string,content:unknown,actorId:string){const workflow=this.workflows.get(workflowId);if(!workflow)throw new Error('WORKFLOW_NOT_FOUND');if(workflow.status!=='MANUAL_HANDOFF_REQUIRED')throw new Error('MANUAL_HANDOFF_STATE_UNSAFE');if(this.orchestrationPlans.has(workflowId))throw new Error('MANUAL_HANDOFF_PLAN_RESUME_UNSUPPORTED');if(this.getArtifacts(workflowId).some(artifact=>artifact.artifactType==='manual_specialist_output'))return workflow;this.addArtifact(workflowId,'manual_specialist_output',content,{specialistId:workflow.logicalSpecialistId??undefined});this.appendEvent({workflowId,eventType:'MANUAL_HANDOFF_ACCEPTED',actorType:'principal',actorId,metadata:{artifactType:'manual_specialist_output',externallySupplied:true}});workflow.status='SPECIALIST_COMPLETE';workflow.version++;workflow.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:'WORKFLOW_SPECIALIST_COMPLETE',actorType:'principal',actorId,metadata:{externallySupplied:true,artifactType:'manual_specialist_output'}});if(workflow.workflowType==='non_code'){workflow.status='COMPLETE';workflow.version++;workflow.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:'WORKFLOW_COMPLETE',actorType:'orchestrator',actorId:'manual-handoff',metadata:{externallySupplied:true}});}return workflow;}
  async acceptManualStageHandoff(workflowId:string,planId:string,planVersion:number,stageId:string,content:unknown,actorId:string){return this.exclusive?this.exclusive(workflowId,async()=>this.acceptManualStageHandoffLocked(workflowId,planId,planVersion,stageId,content,actorId)):this.acceptManualStageHandoffLocked(workflowId,planId,planVersion,stageId,content,actorId);}
  private async acceptManualStageHandoffLocked(workflowId:string,planId:string,planVersion:number,stageId:string,content:unknown,actorId:string){const workflow=this.workflows.get(workflowId);if(!workflow)throw new Error('WORKFLOW_NOT_FOUND');const plan=this.orchestrationPlans.get(planId);if(!plan||plan.workflowId!==workflowId||plan.version!==planVersion)throw new Error('MANUAL_STAGE_HANDOFF_PLAN_MISMATCH');const stage=this.orchestrationPlanStages.get(planId)?.find(item=>item.id===stageId);if(!stage||stage.workflowId!==workflowId)throw new Error('MANUAL_STAGE_HANDOFF_STAGE_MISMATCH');if(stage.status==='COMPLETE'){const existing=stage.outputArtifactId?this.artifacts.get(stage.outputArtifactId):undefined;if(existing?.artifactType==='manual_specialist_output'&&existing.contentHash===createHash('sha256').update(JSON.stringify(content)).digest('hex'))return workflow;throw new Error('MANUAL_STAGE_HANDOFF_ALREADY_ACCEPTED');}if(workflow.status!=='MANUAL_HANDOFF_REQUIRED')throw new Error('MANUAL_STAGE_HANDOFF_STATE_UNSAFE');if(plan.status==='COMPLETE'||plan.status==='FAILED')throw new Error('MANUAL_STAGE_HANDOFF_PLAN_INACTIVE');if(stage.status!=='MANUAL_HANDOFF_REQUIRED')throw new Error('MANUAL_STAGE_HANDOFF_STAGE_NOT_READY');const artifact=this.addArtifact(workflowId,'manual_specialist_output',content,{planId,planStageId:stageId,specialistId:stage.specialistId});stage.status='COMPLETE';stage.outputArtifactId=artifact.id;this.appendEvent({workflowId,eventType:'MANUAL_STAGE_HANDOFF_ACCEPTED',actorType:'principal',actorId,stageId,metadata:{planId,planVersion,planStageId:stageId,specialistId:stage.specialistId,artifactId:artifact.id,externallySupplied:true}});const remaining=this.orchestrationPlanStages.get(planId)!.some(item=>item.status!=='COMPLETE');plan.status=remaining?'RUNNING':'COMPLETE';plan.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:`ORCHESTRATION_STAGE_${stage.status}`,actorType:'orchestrator',actorId:'manual-handoff',stageId,metadata:{planId,planVersion,artifactId:artifact.id,externallySupplied:true}});if(remaining){workflow.status='SPECIALIST_RUNNING';workflow.version++;workflow.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:'WORKFLOW_SPECIALIST_RUNNING',actorType:'orchestrator',actorId:'manual-handoff',stageId,metadata:{planId,planVersion}});}else{workflow.status='SPECIALIST_COMPLETE';workflow.version++;workflow.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:'WORKFLOW_SPECIALIST_COMPLETE',actorType:'orchestrator',actorId:'manual-handoff',stageId,metadata:{planId,planVersion,externallySupplied:true}});if(workflow.workflowType==='non_code'){workflow.status='COMPLETE';workflow.version++;workflow.updatedAt=new Date().toISOString();this.appendEvent({workflowId,eventType:'WORKFLOW_COMPLETE',actorType:'orchestrator',actorId:'manual-handoff',metadata:{planId,planVersion,externallySupplied:true}});}}return workflow;}
  ...
  async persistRoutingResolution(decision:RoutingDecision,candidates:RoutingCandidate[],clarificationQuestion?:string){
    ...
    const catalog=await this.getEffectiveSpecialists();
    if(decision.selectedSpecialistId&&!catalog[decision.selectedSpecialistId]||candidates.some(candidate=>!catalog[candidate.specialistId]))throw new Error('INVALID_ROUTING_TARGET');
    ...
    if(decision.routingConfidence==='AMBIGUOUS'){
      ...
      if(!catalog[decision.selectedSpecialistId])throw new Error('INVALID_ROUTING_TARGET');
    }
    ...
    if(workflow.status==='CREATED'&&decision.routingConfidence!=='NO_MATCH'){
      if(!decision.selectedSpecialistId)throw new Error('ROUTING_SPECIALIST_REQUIRED');
      const target=(await this.getEffectiveSpecialists())[decision.selectedSpecialistId];
