...
export interface Workflow { id:string; requestId:string; workflowType:WorkflowType; logicalSpecialistId:string|null; runtimeId?:string; status:WorkflowStatus; validationRequired?:boolean; effectiveClassification?:'PUBLIC'|'INTERNAL'|'CONFIDENTIAL'|'RESTRICTED'; objective:string; context:unknown; requiresImplementation:boolean; failureCode?:string; failureMessage?:string; version:number; createdAt:string; updatedAt:string; }
