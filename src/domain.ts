...
export interface ManualCodexOutput { implementation_summary:string; changed_files:string[]; test_results:string[]; execution_notes?:string; }
...
export interface SpecialistContribution { artifactId:string; planStageId:string; specialistId:string; purpose:string; artifactType:string; content:unknown; }
export interface MultiSpecialistContext { planId:string; planVersion:number; workflowId:string; objective:string; contributions:SpecialistContribution[]; primaryRequirements:SoftwareOutput; }
...
export function validManualCodexOutput(v:unknown): v is ManualCodexOutput { return isObject(v) && typeof v.implementation_summary==='string' && v.implementation_summary.length<=100000 && Array.isArray(v.changed_files) && v.changed_files.every(item=>typeof item==='string'&&item.length<=1000) && Array.isArray(v.test_results) && v.test_results.every(item=>typeof item==='string'&&item.length<=2000) && (v.execution_notes===undefined || (typeof v.execution_notes==='string'&&v.execution_notes.length<=100000)); }
