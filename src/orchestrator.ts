...
import {CodexPrompt,SoftwareOutput,validCodexPrompt,validManualCodexOutput,validSoftwareOutput,validValidation,Workflow} from './domain.js';
...
export class Orchestrator {
 ...
 async acceptManualHandoff(id:string,content:unknown,principal:ExecutionContext['principal']){const w=await this.store.getWorkflow(id);if(!w)throw new Error('WORKFLOW_NOT_FOUND');assertWorkflowPrincipal(w,principal);if(w.status!=='MANUAL_HANDOFF_REQUIRED'||w.failureCode?.startsWith('CODEX_'))throw new Error('MANUAL_HANDOFF_STATE_UNSAFE');if((await (this.store as Partial<OrchestrationPlanStore>).getOrchestrationPlan?.(id)))throw new Error('MANUAL_HANDOFF_PLAN_RESUME_UNSUPPORTED');const specialist=resolveRoutableSpecialist(w.logicalSpecialistId??'',await this.specialistCatalog());const url=specialist?.chatgptUrl;if(!specialist||specialist.status!=='ACTIVE'||specialist.runtimeStatus==='ACTIVE'||typeof url!=='string'||!/^https:\/\/(?:chatgpt\.com|chat\.openai\.com)\/g\//.test(url))throw new Error('MANUAL_HANDOFF_UNAVAILABLE');if(w.workflowType==='software'&&!validSoftwareOutput(content))throw new Error('INVALID_MANUAL_HANDOFF_OUTPUT');if(w.workflowType!=='software'&&(typeof content!=='string'&&(!content||typeof content!=='object'||Array.isArray(content))))throw new Error('INVALID_MANUAL_HANDOFF_OUTPUT');return this.store.acceptManualHandoff(id,content,principal.id);}
 async acceptManualCodexHandoff(id:string,content:unknown,principal:ExecutionContext['principal']){const w=await this.store.getWorkflow(id);if(!w)throw new Error('WORKFLOW_NOT_FOUND');assertWorkflowPrincipal(w,principal);if(!validManualCodexOutput(content))throw new Error('INVALID_CODEX_HANDOFF_OUTPUT');return this.store.acceptManualCodexHandoff(id,content,principal.id);}
 async acceptManualStageHandoff(id:string,planId:string,stageId:string,content:unknown,principal:ExecutionContext['principal']){const w=await this.store.getWorkflow(id);if(!w)throw new Error('WORKFLOW_NOT_FOUND');assertWorkflowPrincipal(w,principal);const plans=this.store as PersistenceStore&Partial<OrchestrationPlanStore>;const plan=await plans.getOrchestrationPlan?.(id);if(!plan||plan.id!==planId)throw new Error('MANUAL_STAGE_HANDOFF_PLAN_MISMATCH');const stages=await plans.getOrchestrationPlanStages?.(plan.id);const stage=stages?.find(item=>item.id===stageId);if(!stage||!['MANUAL_HANDOFF_REQUIRED','COMPLETE'].includes(stage.status))throw new Error('MANUAL_STAGE_HANDOFF_STAGE_NOT_READY');if(stage.status==='COMPLETE'){const hash=createHash('sha256').update(JSON.stringify(content)).digest('hex');const existing=(await this.store.getArtifacts(id)).find(item=>item.id===stage.outputArtifactId&&item.planId===plan.id&&item.planStageId===stage.id&&item.artifactType==='manual_specialist_output');if(existing?.contentHash===hash)return w;throw new Error('MANUAL_STAGE_HANDOFF_ALREADY_ACCEPTED');}if(w.status!=='MANUAL_HANDOFF_REQUIRED')throw new Error('MANUAL_STAGE_HANDOFF_STATE_UNSAFE');const specialist=resolveRoutableSpecialist(stage.specialistId,await this.specialistCatalog());const url=specialist?.chatgptUrl;if(!specialist||specialist.status!=='ACTIVE'||specialist.runtimeStatus==='ACTIVE'||typeof url!=='string'||!/^https:\/\/(?:chatgpt\.com|chat\.openai\.com)\/g\//.test(url))throw new Error('MANUAL_STAGE_HANDOFF_UNAVAILABLE');const raw=w.context as any;const repositoryId=raw?.resolvedRepository?.repositoryId??raw?.repositoryId;const repo=repositoryId&&this.registry?resolveRepository(repositoryId,this.registry):raw?.resolvedRepository;authorize({caller:principal.id,operation:'run',specialistId:specialist.specialistId,repositoryId,executionType:'specialist',dataClassification:w.effectiveClassification??raw?.effectiveClassification??'PUBLIC',principalMaxClassification:principal.maxClassification,repository:repo});if(w.workflowType==='software'&&!validSoftwareOutput(content))throw new Error('INVALID_MANUAL_HANDOFF_OUTPUT');if(w.workflowType!=='software'&&(typeof content!=='string'&&(!content||typeof content!=='object'||Array.isArray(content))))throw new Error('INVALID_MANUAL_HANDOFF_OUTPUT');return this.store.acceptManualStageHandoff(id,plan.id,plan.version,stage.id,content,principal.id);}
