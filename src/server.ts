...
import { ApprovalStore, ClarificationStore, MemoryStore, OrchestrationPlanStore, PersistenceStore } from './store.js';
...
const server=createServer(async(req,res)=>{try{
  ...
  const stageHandoffMatch=req.url?.match(/^\/v1\/workflows\/([^/]+)\/plans\/([^/]+)\/stages\/([^/]+)\/handoff-response$/);
  if(stageHandoffMatch&&req.method==='POST'){
    const body=await requestBody(req);assertAllowedKeys(body,['output']);if(body.output===undefined)throw new Error('INVALID_REQUEST');
    const updated=await app.acceptManualStageHandoff(decodeURIComponent(stageHandoffMatch[1]),decodeURIComponent(stageHandoffMatch[2]),decodeURIComponent(stageHandoffMatch[3]),body.output,principal);
    return json(res,200,await persistedReadModel(updated.id,principal));
  }
  ...
  if(req.method==='POST'&&action==='run'){const activePlan=await (store as Partial<OrchestrationPlanStore>).getOrchestrationPlan?.(id);const executionType=activePlan&&activePlan.status!=='COMPLETE'?'specialist':workflow.status==='ROUTED'?'specialist':'codex';const current=contextFromWorkflow(workflow,'run',executionType,principal,config.repositories as any);return json(res,200,await app.run(id,current,req.headers['x-worker-id']?.toString()??'http-worker'));}
  ...
  if(req.method==='GET'&&action==='handoff'){const model=await persistedReadModel(id,principal);return json(res,200,model.codexHandoff??model.manualHandoff??{available:false,specialistId:workflow.logicalSpecialistId});}
  if(req.method==='POST'&&action==='handoff-response'){const body=await requestBody(req);assertAllowedKeys(body,['output']);if(body.output===undefined)throw new Error('INVALID_REQUEST');if(workflow.failureCode==='CODEX_RUNTIME_UNAVAILABLE')await app.acceptManualCodexHandoff(id,body.output,principal);else await app.acceptManualHandoff(id,body.output,principal);return json(res,200,await persistedReadModel(id,principal));}
