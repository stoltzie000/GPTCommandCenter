...
The MVP client is served by the same backend at `/`. For local development, set `APP_MODE=local`, run `npm run build && npm start`, and open `http://localhost:8080/`. Enter a configured repository id and describe the desired outcome; specialist selection remains server-controlled. In deployed token mode, the client accepts a bearer token in memory for the current page session only and never bundles or stores a privileged token. The backend remains authoritative for authorization, routing, execution, and workflow state.

The client shows clarification and approval prompts, automatic execution progress, orchestration stages, results, and evidence. For a manual-only specialist it presents the configured ChatGPT navigation link and a bounded copyable handoff package. Opening that link does not transfer data automatically; returned text is submitted through the authorized workflow endpoint and is labeled externally supplied. Multi-specialist plans that require unsupported stage-level manual return show manual intervention guidance without exposing a misleading submission form.

Multi-specialist plans now expose an actionable stage-level handoff when the backend marks a specific stage as `MANUAL_HANDOFF_REQUIRED`. The return endpoint is bound to the workflow, plan version, stage, and trusted specialist; accepted output is stored as externally supplied evidence, and dependent stages resume only through the normal backend run/continue action. Run `npm run test:browser` for the optional Playwright acceptance journey, or `npm run test:browser-certify` for mandatory real-browser certification; the latter fails when Playwright or a browser binary is unavailable.

For software delivery, Codex Prompt Builder persists a `codex_prompt` artifact before execution. If the approved Codex runtime is unavailable, the workflow enters a Codex-specific manual handoff state instead of reporting prompt generation as failed. The client displays a bounded, copyable task package through the existing `/v1/workflows/:id/handoff-response` lifecycle. A returned report is stored as `manual_codex_output`, labeled externally supplied, and requires review; it is not proof of repository changes or independently validated implementation. A runtime that was actually attempted and failed remains a genuine `FAILED` execution outcome.

## Security and limitations

Registry entries are server-controlled; ChatGPT URLs are navigation-only and are never treated as executable endpoints. Client status/runtime/evidence fields are ignored. Only trusted executor return facts or explicitly labeled user-submitted manual-handoff artifacts can advance a workflow. Manual-only/unverified runtimes hand off without execution events; a configured custom-GPT handoff exposes a bounded task package and authorized return endpoint. `APP_MODE=deployed` fails closed until the PostgreSQL store is wired as the runtime authority; it never silently downgrades to `MemoryStore`. Local mode is explicitly selected with `APP_MODE=local` and may use the memory test adapter. OpenAI, Codex CLI, PostgreSQL, Docker/rootless-container execution, and approved repository configuration are external prerequisites and were not live-verified in this Windows environment. The rootless container adapter uses the observed Codex 0.152.0 `codex exec` contract and defaults to read-only root, no network, dropped capabilities, no-new-privileges, resource limits, and an explicit workspace mount.
