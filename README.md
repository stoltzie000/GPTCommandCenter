...
Multi-specialist plans now expose an actionable stage-level handoff when the backend marks a specific stage as `MANUAL_HANDOFF_REQUIRED`. The return endpoint is bound to the workflow, plan version, stage, and trusted specialist; accepted output is stored as externally supplied evidence, and dependent stages resume only through the normal backend run/continue action. Run `npm run test:browser` for the Playwright acceptance journey when Playwright and a browser binary are installed; the test is intentionally skipped when that optional tooling is unavailable.
