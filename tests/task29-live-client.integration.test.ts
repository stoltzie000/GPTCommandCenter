...
async function startServer(){
  ...
  while(Date.now()<deadline){try{const response=await fetch(`http://127.0.0.1:${port}/ready`);if(response.status===200)return {child,port};}catch{}await new Promise(resolve=>setTimeout(resolve,100));}
  child.kill('SIGTERM');throw new Error('TASK29_SERVER_START_TIMEOUT');
}
...
async function submit(page:any,repository:string,objective:string,type:'software'|'non_code',implementation=false){await page.locator('#repository').fill(repository);await page.locator('#objective').fill(objective);await page.locator('#workflow-type').selectOption(type);const checkbox=page.locator('#requires-implementation');if(implementation)await checkbox.check();await page.locator('#task-form button[type="submit"]').click();await page.locator('#workflow-panel').waitFor();}
...
if(!playwright||!browserAvailable){
  ...
  test('real Chromium client drives the real GPTCC server through manual return and Codex handoff boundary',async()=>{
    const {child,port}=await startServer();const context=await playwright.chromium.launch({headless:true});
    try{
      const browserContext=await context.newContext();await browserContext.grantPermissions(['clipboard-read','clipboard-write'],{origin:`http://127.0.0.1:${port}`});const page=await browserContext.newPage();await page.goto(`http://127.0.0.1:${port}/`);
      await submit(page,'task29-multi','Review architecture, security, and repository configuration','non_code');await page.locator('#next-actions button').click();await page.locator('.stage-handoff').waitFor();
