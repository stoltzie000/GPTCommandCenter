...
if(!playwright||!browserAvailable){
  ...
  test('real Chromium client drives the real GPTCC server through manual return and Codex handoff boundary',async()=>{
    ...
    try{
      const browserContext=await context.newContext();await browserContext.grantPermissions(['clipboard-read','clipboard-write'],{origin:`http://127.0.0.1:${port}`});const page=await browserContext.newPage();page.on('console',(message:any)=>{if(message.type()==='error')console.error(`browser-console: ${message.text()}`);});page.on('response',async(response:any)=>{if(response.status()>=400)console.error(`browser-response: ${response.status()} ${response.url()} ${await response.text().catch(()=> '')}`);});await page.goto(`http://127.0.0.1:${port}/`);
