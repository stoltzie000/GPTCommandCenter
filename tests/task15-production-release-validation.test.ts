...
test('Task 15 migration set is ordered, complete, and repeatable by construction', () => {
  const files = readdirSync(migrationDirectory).filter(file => /^00[1-8]_.*\.sql$/.test(file)).sort();
  assert.deepEqual(files.map(file => file.slice(0, 3)), ['001', '002', '003', '004', '005', '006', '007', '008']);
